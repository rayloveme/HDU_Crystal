...
//    piout=0;
    pid->value = pid->Kp / 100.0 * (pid->err) + pid->Kd / 100.0 * (pid->err - pid->err_1)
    + piout;
...
int IncPid_Cal(struct Pid *pid, int act, int set)
{
    ...
    if ((pid->err < 1000) && (pid->err > -1000))
        ...
        pid_temp = pid->Kp / 1000.0 * (pid->err - pid->err_1) + pid->Ki / 1000.0 * pid->err + pid->Kd / 1000.0 * (pid->err - 2 * pid->err_1 + pid->err_2);
    pid->value += pid_temp;
